...
import { mapStockFromStatement } from "@/lib/stock-mapping";
import { StockHolding } from "@/types/finance";

export function FetchStocksDialog() {
  const [open, setOpen] = useState(false);
  const [isParsing, setIsParsing] = useState(false);
  const [previewData, setPreviewData] = useState<Omit<StockHolding, "id">[]>([]);
  const [error, setError] = useState<string | null>(null);
  const [authPromptOpen, setAuthPromptOpen] = useState(false);
  
  const { isLoggedIn } = useAuthStore();
  const fileInputRef = useRef<HTMLInputElement>(null);
  const { bulkUpsertHoldings } = useStocksStore();
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (event) => {
      try {
        ...
        const colMap = {
          name: findCol(["stock name", "particulars", "symbol"]),
          qty: findCol(["quantity", "qty"]),
          buyPrice: findCol(["avg price", "average buy", "buy price"]),
          currPrice: findCol(["closing price", "ltp", "current price"]),
        };

        if (colMap.name === -1 || colMap.qty === -1) {
          throw new Error("Required columns (Name/Quantity) not found. This file format might be unsupported.");
        }

        const mappedHoldings: Omit<StockHolding, "id">[] = dataRows
          .filter(row => row[colMap.name] && !isNaN(parseFloat(String(row[colMap.qty]))))
          .map(row => {
            const rawName = String(row[colMap.name]);
            const mapping = mapStockFromStatement(rawName);
            
            return {
              symbol: mapping.symbol || rawName,
              name: rawName,
              quantity: cleanCurrency(row[colMap.qty]),
              avgBuyPrice: colMap.buyPrice > -1 ? cleanCurrency(row[colMap.buyPrice]) : 0,
              currentPrice: colMap.currPrice > -1 ? cleanCurrency(row[colMap.currPrice]) : 0,
              sector: mapping.sector || "Others",
  ...
  const handleImport = async () => {
    ...
    await bulkUpsertHoldings(previewData);
    setOpen(false);
    setPreviewData([]);
    setError(null);
  };
              ...
              <div className="flex items-center justify-between">
                <div>
                  <DialogTitle className="text-4xl font-black tracking-tight bg-gradient-to-r from-primary to-primary/40 bg-clip-text text-transparent flex items-center gap-3">
                    <TrendingUp className={isParsing ? "animate-spin text-primary" : "text-primary"} size={32} />
                    Auto Stocks Sync
                  </DialogTitle>
                ...
                <div className="hidden md:flex items-center gap-6">
                  <div className="text-right">
                    <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">Security Status</p>
                    <div className="flex items-center gap-1.5 text-primary">
                      <ShieldCheck size={14} />
                      <span className="text-xs font-bold uppercase tracking-tighter">Verified Browser Env</span>
                    </div>
                  </div>
                </div>
              </div>
                ...
                <div className="space-y-8 animate-in fade-in slide-in-from-bottom-8 duration-700">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div className="bg-foreground/5 border border-border/20 p-6 rounded-3xl">
                      <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground mb-1">Total Stocks</p>
                    ...
                    <div className="bg-primary/5 border border-primary/20 p-6 rounded-3xl">
                      ...
                      <p className="text-2xl font-black text-primary">
                        {formatINR(previewData.reduce((acc, curr) => acc + (curr.quantity * curr.avgBuyPrice), 0))}
                      </p>
                    </div>
                    <div className="bg-foreground/5 border border-border/20 p-6 rounded-3xl">
                      <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground mb-1">Market Value</p>
                      <p className="text-2xl font-black text-foreground">
                        {formatINR(previewData.reduce((acc, curr) => acc + (curr.quantity * curr.currentPrice), 0))}
                      </p>
                    </div>
                  </div>
                      ...
                      <table className="w-full text-sm border-collapse">
                        <thead>
                          <tr className="bg-foreground/5 text-muted-foreground">
                            <th className="px-8 py-5 text-left text-[10px] font-black uppercase tracking-widest border-b border-border/10">Stock / Symbol</th>
                            <th className="px-8 py-5 text-right text-[10px] font-black uppercase tracking-widest border-b border-border/10">Quantity</th>
                            <th className="px-8 py-5 text-right text-[10px] font-black uppercase tracking-widest border-b border-border/10">Avg Buy</th>
                            <th className="px-8 py-5 text-right text-[10px] font-black uppercase tracking-widest border-b border-border/10">LTP</th>
                            <th className="px-8 py-5 text-right text-[10px] font-black uppercase tracking-widest border-b border-border/10">P&L</th>
                          </tr>
                        </thead>
                        <tbody className="divide-y divide-border/10">
                          {previewData.slice(0, 10).map((stk, i) => {
                            const pnl = (stk.currentPrice - stk.avgBuyPrice) * stk.quantity;
                            return (
                              <tr key={i} className="group hover:bg-primary/[0.03] transition-colors">
                                <td className="px-8 py-6">
                                  <div className="font-bold text-foreground text-sm group-hover:text-primary transition-colors">{stk.symbol}</div>
                                  <div className="text-[10px] text-muted-foreground font-black uppercase tracking-widest mt-1 opacity-60 truncate max-w-[200px]">{stk.name}</div>
                                </td>
                                <td className="px-8 py-6 text-right font-mono font-black text-foreground leading-none">{stk.quantity}</td>
                                <td className="px-8 py-6 text-right font-mono text-muted-foreground/80 leading-none">{formatINR(stk.avgBuyPrice)}</td>
                                <td className="px-8 py-6 text-right font-mono font-bold text-foreground leading-none">{formatINR(stk.currentPrice)}</td>
                    ...
                    {previewData.length > 10 && (
                      <div className="p-4 bg-muted/30 text-center border-t border-border/10">
                        <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">
                          + {previewData.length - 10} more stocks hidden in preview
                        </p>
                      </div>
                    )}
                  </div>
                ...
                <div className="space-y-12 py-4">
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center gap-2 mb-4">
                      <PieChart className="text-primary" size={16} />
                      <p className="text-xs font-black uppercase tracking-[0.2em] text-primary">Broker Import Instructions</p>
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      {[
                        { 
                          name: "Groww", 
                          steps: "Portfolio > Stocks > Reports > Holding Statement",
                          color: "from-emerald-500/10 to-transparent"
                        },
                        { 
                          name: "Zerodha", 
                          steps: "Console > Portfolio > Holdings > Download XLSX",
                          color: "from-blue-500/10 to-transparent"
                        },
                        { 
                          name: "Upstox", 
                          steps: "Reports > Holding Statement > Download Excel",
                  ...
                  >
                    ...
                    <div className="text-center relative">
                      <p className="text-3xl font-black text-foreground tracking-tight mb-2">Sync Account Statement</p>
                      <p className="text-sm text-muted-foreground font-medium">Drop your holding XLSX here or click to browse</p>
                    </div>
                    ...
                    <div className="flex gap-2 text-[10px] font-black text-primary uppercase tracking-[0.2em] bg-primary/10 px-6 py-2.5 rounded-full mt-4 border border-primary/20 group-hover:bg-primary group-hover:text-black transition-all">
                      <FileSpreadsheet size={14} /> Local Parsing Only
                    </div>
                    ...
                    {isParsing && (
                      <div className="absolute inset-0 bg-card/80 backdrop-blur-md rounded-[40px] z-50 flex flex-col items-center justify-center gap-4">
                        ...
                        <p className="text-xl font-black text-primary tracking-widest uppercase">Syncing Records...</p>
                        <p className="text-xs text-muted-foreground font-medium">Building your professional portfolio view</p>
                      </div>
                    )}
                  </div>
                  ...
                  {error && (
                    <div className="bg-red-500/10 border border-red-500/20 p-6 rounded-3xl flex items-center gap-4 text-red-500 animate-in slide-in-from-top-4">
                      ...
                      <div>
                        <p className="font-black uppercase text-xs tracking-widest mb-1">Upload Error</p>
                  ...
                  <div className="flex items-center justify-center gap-12 border-t border-border/10 pt-8 opacity-40 grayscale hover:grayscale-0 hover:opacity-100 transition-all duration-700">
                     <div className="flex items-center gap-2">
                        ...
                        <span className="text-[10px] font-black uppercase tracking-widest">Real-time Parsing</span>
                     </div>
                     <div className="flex items-center gap-2">
                        <Building2 size={18} className="text-primary" />
                        <span className="text-[10px] font-black uppercase tracking-widest">Broker Agnostic</span>
                     </div>
                     <div className="flex items-center gap-2">
                        <ShieldCheck size={18} className="text-primary" />
                        <span className="text-[10px] font-black uppercase tracking-widest">Statement Verified</span>
                     </div>
                  </div>
                </div>
              )}
