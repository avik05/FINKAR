        ...
        <FinanceCard className="w-full flex flex-col">
          <div className="p-6 border-b border-border/50">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
              <h2 className="text-lg font-heading font-semibold text-foreground">Transaction Ledger</h2>
              <div className="flex items-center gap-2 w-full sm:w-auto">
                <div className="relative flex-1 sm:w-64">
                  <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
                  <Input type="search" placeholder="Search transactions..." className="pl-10 bg-foreground/5 border-border/50 text-sm h-10" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
                </div>
              </div>
            </div>
          </div>
          ...
          {transactions.length === 0 ? (
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="border-b border-border/50 text-[10px] uppercase tracking-wider text-muted-foreground bg-foreground/5">
                    <th className="hidden sm:table-cell px-6 py-4 font-bold">Date</th>
                    <th className="px-6 py-4 font-bold">Merchant / Details</th>
                    <th className="hidden sm:table-cell px-6 py-4 font-bold">Category</th>
                    <th className="hidden sm:table-cell px-6 py-4 font-bold">Account</th>
                    ...
                    <tr key={tx.id} className="hover:bg-foreground/5 transition-colors group">
                      ...
                      <td className="px-6 py-4">
                        <div className="flex justify-end gap-1">
                          ...
                          <button onClick={() => handleDeleteTransaction(tx.id)} className="p-1.5 rounded-lg opacity-0 group-hover:opacity-100 hover:bg-destructive/20 text-destructive transition-all" title="Delete">
                            <Trash2 size={14} />
                          </button>
                        </div>
                      </td>
